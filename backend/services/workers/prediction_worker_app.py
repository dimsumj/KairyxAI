...
def _startup() -> None:
    ...
    start_queue_poller(app, service_role="prediction-worker", handler=_handle_queue_payload)
...
def _run_prediction_job(job_id: str) -> dict:
    init_db()
...
async def handle_pubsub_push(request: Request) -> dict:
    ...
    return _run_prediction_job(job_id)
