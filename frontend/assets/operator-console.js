// KairyxAI operator console runtime. Extracted from index.html for frontend hardening.
        document.addEventListener('DOMContentLoaded', function () {
            ...
            async function networkRequest(path, options = {}) {
                const { method = 'GET', body, headers = buildApiHeaders(Boolean(body)) } = options;
            ...
            async function mockStorageRequest(path, options = {}) {
                const { method = 'GET', body, headers = buildApiHeaders(Boolean(options.body)) } = options;
                const normalizedMethod = String(method || 'GET').toUpperCase();
                const { segments, searchParams } = parseApiPath(path);
                const primary = String(segments[0] || '');
                const tenantId = String(headers['x-tenant-id'] || headers['X-Kairyx-Tenant'] || 'default').trim() || 'default';
                ...
                if (primary === 'imports') {
                    ...
                    if (segments.length >= 2) {
                        ...
                        if (segments[2] === 'run' && normalizedMethod === 'POST') {
                            ...
                            const job = state.imports.find((item) => item.id === jobId);
                            if (!job) {
                                throw createMockRequestError(`Import job '${jobId}' not found.`, 404);
                            }
                            return job;
            ...
            async function apiRequest(path, options = {}) {
                ...
                const headers = options.headers || buildApiHeaders(Boolean(body));
