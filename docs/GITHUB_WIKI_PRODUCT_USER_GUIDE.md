...
1. Expand delivered pgvector live-sync receipts and saved-query artifact retrieval into additional provider-native vector sync/query paths and structured artifact families.
...
Data Core now owns the RAG knowledge-ingestion slice through `Data Core -> Knowledge` and `/api/v1/knowledge`. Operators can upload text or markdown playbooks, campaign briefs, SOPs, reports, and FAQs from the console, run cited evidence checks, include saved-query artifacts, review vector-store readiness, and export full setup artifacts with `Export .json` buttons instead of reading raw JSON/code fields. Ask AI consumes `hybrid_v1` retrieval evidence packs for relevant setup, diagnostics, strategy, and copy-drafting prompts, and evidence artifacts open back into the Knowledge page. Backend deployments can configure embedding/vector metadata with `KNOWLEDGE_EMBEDDING_PROVIDER`, `KNOWLEDGE_EMBEDDING_MODEL`, `KNOWLEDGE_VECTOR_STORE`, `KNOWLEDGE_VECTOR_INDEX`, `KNOWLEDGE_VECTOR_NAMESPACE`, and `KNOWLEDGE_VECTOR_SECRET_REF`; production external providers/stores require the secret reference and exports expose only redacted vector-index records. pgvector can live-sync upsert/archive receipts when the secret reference resolves to a PostgreSQL DSN or a JSON object with `dsn`, optional `schema`, and optional `table`; provider failures are sanitized and local control-plane retrieval remains available. Pinecone, Qdrant, Weaviate, Milvus, OpenSearch, BigQuery Vector, and custom gateways still produce managed shadow receipts until their live adapters are added.
...
| `Include saved queries` | Checkbox | Add SQL Workspace saved-query artifacts to the evidence search when cohort or audience setup context matters. | The evidence cards can include `Saved Query` citations with module links and structured summaries. |
...
| `POST /api/v1/knowledge/retrievals` | Run a tenant/project-scoped retrieval over knowledge chunks and optional saved-query artifacts. | Supports `retrieval_mode` values `lexical_v1` and `hybrid_v1`; set `include_product_artifacts=true` with `artifact_types=["saved_query"]` to include structured SQL Workspace citations. Returns ranked citations, snippets, full cited text, semantic/rerank signals, feedback boost/ranking signals, artifact summaries, vector-index metadata, a context pack, and an export descriptor. |
...
| `GET /api/v1/knowledge/vector-indexes/{index_id}/export` | Export vector-index metadata. | Returns `knowledge_vector_index.v1`; records include hashes, adapter receipts, live-sync status when available, and embedding metadata but omit raw vectors and secret material. |

Supported `source_type` values are `markdown`, `text`, `campaign_brief`, `sop`, `report`, `faq`, and `playbook`. Supported retrieval `artifact_types` currently include `saved_query`. Supported `visibility` values are `workspace`, `project`, and `private`. Normal users should not paste secret material into knowledge documents; credentials still belong in secure connector/provider/model-profile setup.
...
- Completed one-time scheduled push workflows do not auto-archive anymore. They move into `Sent` and remain there until an operator archives them manually.
...
Use this section to schedule email campaigns, manage push workflows, and review a unified operational summary. The table merges email campaigns, push workflows, and direct one-time push dispatches, then adds a selected-item panel so operators can edit, schedule, archive, or inspect the current record without switching pages.
...
| Direct push item `View` | Row or detail button | Loads the selected one-time push dispatch into the detail panel. | None | Dispatch payload and provider response appear in the detail panel. |
| Direct push item `Archive` | More action | Archives a completed one-time push dispatch from `Sent` or `All`. | None | The dispatch moves into `Archived`. |
...
- Unsent scheduled push workflows do not show an `Archive` action.
- Published recurring push workflows stay in `Scheduled` even after a successful run. They only leave that filter when they are paused, changed into another state, or archived through a later lifecycle change.
...
| `POST /api/v1/experiments/ai-evaluations/judge-runs` | Record a model-judge or offline evaluation batch. | Unscored `model_judge` items use the configured Ask AI runtime; scored `model_judge` or `offline_eval` items can be recorded from an external review or benchmark without exposing JSON fields in the console. |
