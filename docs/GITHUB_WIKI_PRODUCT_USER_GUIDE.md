...
| `Fetch Row Count` | Button | Runs an exact row-count lookup for the selected or manually entered BigQuery table without reloading the whole table list. | `prediction_scores` | Status copy shows the exact count, and the discovered table label is updated for that table. |
