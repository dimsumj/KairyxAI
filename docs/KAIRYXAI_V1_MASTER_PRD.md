...
3. `Retrieval And Citations`: expand delivered hybrid knowledge retrieval, vector-index shadow records, and managed vector-adapter receipts into provider-specific live vector sync plus structured product-artifact retrieval, while preserving query planning, rerank metadata, and citation-backed context packs.
...
2. `Retrieval And Citations`
   - Delivered lexical knowledge evidence packs, config-driven vector-index metadata, managed vector-adapter receipts, persisted vector shadow records, `hybrid_v1` semantic/reranked retrieval, and Ask AI citation attachment for relevant setup, diagnostics, audience, workflow, experiment, and copy-drafting turns
   - Next, extend managed shadow-adapter receipts into provider-specific live vector sync, structured artifact retrieval, and richer module handoff cards
