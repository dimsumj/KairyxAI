...
- `Experiment Hub`: experiment config, assignment, exposure, outcome, summary, integrity checks, decision logging, AI/RAG evaluation telemetry, deterministic auto-grading, model-judge/offline evaluation runs, scheduled AI quality alert checks, AI Quality Monitor, AI feedback records, and feedback-learning profiles with Ask AI handoffs for setup and diagnostics
...
3. `Retrieval And Citations`: expand delivered hybrid knowledge retrieval, vector-index records, managed vector-adapter receipts, pgvector live-sync receipts, and saved-query artifact retrieval into broader provider-native sync/query paths and structured product-artifact families, while preserving query planning, rerank metadata, and citation-backed context packs.
4. `Evaluation And Monitoring`: expand the delivered AI/RAG evaluation telemetry, deterministic grading APIs, model-judge/offline eval adapters, scheduled alert checks, and AI Quality Monitor into recall probes and external alert-routing jobs for retrieval quality, answer relevance, citation coverage, campaign-copy usefulness, and prompt-to-artifact completion rates.
...
2. `Retrieval And Citations`
   - Delivered lexical knowledge evidence packs, config-driven vector-index metadata, managed vector-adapter receipts, persisted vector records, pgvector live-sync receipts, saved-query artifact citations, `hybrid_v1` semantic/reranked retrieval, and Ask AI citation attachment for relevant setup, diagnostics, audience, workflow, experiment, and copy-drafting turns
   - Next, extend live sync/query support to additional vector providers, add more structured artifact families, and enrich module handoff cards
3. `Evaluation And Monitoring`
   - Delivered AI/RAG evaluation telemetry records, summaries, deterministic auto-grading, model-judge/offline evaluation run adapters, scheduled AI quality alert-check artifacts, durable quality-alert lifecycle resources, and the Experiment Hub AI Quality Monitor for retrieval quality, answer relevance, citation coverage, campaign-copy usefulness, prompt-to-artifact completion, feedback diagnostics, coverage gaps, and judge-readiness lanes
   - Next, add recall probes and external alert routing for stale sources, missing evidence, unsupported prompts, and eval drift
4. `Feedback Loop`
