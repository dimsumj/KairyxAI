...
- provider-aware lifecycle email campaigns in `Action Orchestrator`, including tenant-scoped provider connections managed in `Data Core -> Connectors`, SendGrid dynamic template browsing, Braze API campaign selection, prediction-or-cohort audience targeting, human-readable audience labels, sampled JSON-key recipient field selectors, optional prediction risk filters, deeplink merge-field injection, and separate upcoming versus past campaign views
