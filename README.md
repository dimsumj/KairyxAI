...
- tenant/project-scoped knowledge ingestion and retrieval under `/api/v1/knowledge`, with text/markdown campaign briefs, SOPs, reports, FAQs, and playbooks stored as auditable documents and deterministic chunks; each document preserves provenance, tags, content hash, lifecycle status, pending embedding metadata, and an exportable `knowledge_document.v1` JSON artifact, while retrieval runs return cited `knowledge_evidence_pack.v1` artifacts for review
...
- an AI-native prompt-first operator console where module-level starter prompts and the global `Ask AI` session are the primary path for connector setup, mapping fixes, cohort drafts, SQL drafts, campaigns, workflows, experiment configuration, health summaries, diagnostics, and supported lifecycle actions; raw JSON input/output panels are removed from the operator path, structured payloads export through `Export .json`, and only compact selectors, secure credential dialogs, marketer-readable copy fields, and narrow advanced fallbacks remain visible
...
- Experiment Hub AI/RAG evaluation telemetry under `/api/v1/experiments/ai-evaluations`, with tenant/project-scoped records for retrieval quality, citation coverage, answer relevance, campaign-copy usefulness, and prompt-to-artifact completion; summaries expose score averages, outcome rates, and dimension averages, and each record exports as an `ai_evaluation_record.v1` JSON artifact
- Experiment Hub AI feedback records under `/api/v1/experiments/ai-feedback`, with operator approvals, edits, ratings, clicks, sends, workflow results, and experiment outcomes feeding summary metrics and deterministic retrieval ranking boosts for knowledge chunks or documents
...
Against the target growth-marketing RAG architecture, the current product already covers structured data ingestion, knowledge document chunking, lexical retrieval evidence packs, feedback-boosted ranking, AI/RAG evaluation telemetry, mappings, cohort/action/experiment operations, model profile management, Ask AI handoffs, artifacts, audit history, and secure credential setup. The highest-priority completion gaps are semantic/vector retrieval wired into Ask AI answers, automated retrieval/generation grading, model-based learning from feedback, and a richer no-code UI for knowledge intake.
