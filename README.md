...
- provider-aware lifecycle delivery in `Action Orchestrator`, including tenant-scoped provider connections managed in `Data Core -> Connectors` through a `Connect Campaign Provider` action, row-level provider deletion guarded against active campaign references, SendGrid dynamic template browsing, Braze API campaign selection, immediate single-user push sends, provider-backed push workflow delivery, prediction-or-cohort audience targeting, human-readable audience labels, sampled JSON-key recipient field selectors, optional prediction risk filters, deeplink merge-field injection, push payload fields for `campaign_name`, `title`, `body`, `deep_link`, `deep_link_token`, JSON `data`, optional schedule override, simulator fallback when no push provider connection is selected, and separate upcoming versus past email campaign views
- a guided `Audience Engine` builder that defaults marketers into selector-driven cohort construction, supports prediction-led audiences by source or explicit run, previews combined versus split cohort output, keeps raw SQL behind an `Advanced SQL` disclosure, and stores builder provenance on draft cohorts while preserving existing cohort CRUD and SQL workspace APIs
- a prompt-driven `Insight Copilot` operator agent that can use backend-managed Gemini, OpenAI, or Anthropic model profiles, with Gemini, LM Studio, Ollama, and custom OpenAI-compatible presets managed from `Data Core -> Connectors -> AI Agents & Models`; OpenAI-compatible endpoints must be reachable from the backend runtime, so localhost-style presets are for self-hosted or local deployments; the agent can draft SQL from prediction context, draft guided audience-builder states, reuse or start prediction jobs, and create draft cohorts, email campaigns, and workflows from one chat flow while keeping publish, send, and other live actions confirmation-gated or manual
