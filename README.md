...
- a Figma-derived SaaS shell with a responsive sidebar, inline expanding section lists, a search-only top bar, and a Settings page
...
- Settings-hosted workspace shortcuts plus session, API key, and application-status controls so the header stays focused on search and the left rail stays navigation-only
...
- backend-served React shell with a Figma-based SaaS layout, inline section expansion, an icon-only collapsed rail with right-side section popouts, a search-only top bar, and Settings-driven workspace and session control
