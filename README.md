...
- tenant/project-scoped knowledge ingestion and retrieval under `/api/v1/knowledge`, with text/markdown campaign briefs, SOPs, reports, FAQs, and playbooks stored as auditable documents and deterministic chunks; each document preserves provenance, tags, content hash, lifecycle status, local semantic-vector metadata, and an exportable `knowledge_document.v1` JSON artifact, while retrieval runs support compatibility `lexical_v1` and hybrid semantic/reranked `hybrid_v1` evidence packs that Ask AI attaches to relevant strategy, setup, diagnostics, and copy-drafting turns
...
Against the target growth-marketing RAG architecture, the current product already covers structured data ingestion, knowledge document chunking, local semantic-vector metadata, Ask AI hybrid retrieval-grounded evidence packs, feedback-boosted ranking, AI/RAG evaluation telemetry, mappings, cohort/action/experiment operations, model profile management, Ask AI handoffs, artifacts, audit history, and secure credential setup. The highest-priority completion gaps are automated retrieval/generation grading, model-based learning from feedback, richer no-code UI for knowledge intake, and provider-grade embedding/vector backends beyond the deterministic local index.
