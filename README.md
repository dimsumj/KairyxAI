...
- tenant/project-scoped knowledge ingestion and retrieval under `/api/v1/knowledge`, with text/markdown campaign briefs, SOPs, reports, FAQs, and playbooks stored as auditable documents and deterministic chunks; each document preserves provenance, tags, content hash, lifecycle status, pending embedding metadata, and an exportable `knowledge_document.v1` JSON artifact, while retrieval runs return cited `knowledge_evidence_pack.v1` artifacts that Ask AI now attaches to relevant strategy, setup, diagnostics, and copy-drafting turns
...
Against the target growth-marketing RAG architecture, the current product already covers structured data ingestion, knowledge document chunking, Ask AI retrieval-grounded lexical evidence packs, feedback-boosted ranking, AI/RAG evaluation telemetry, mappings, cohort/action/experiment operations, model profile management, Ask AI handoffs, artifacts, audit history, and secure credential setup. The highest-priority completion gaps are semantic/vector retrieval and reranking, automated retrieval/generation grading, model-based learning from feedback, and a richer no-code UI for knowledge intake.
