...
- connector-first Data Core onboarding, including a `Connect Data Source` entry point from the main workbench plus `AI Agents & Models` and `MCP Connections` sections in `Data Core -> Connectors` for Ask AI runtime setup and remote MCP server access
...
- actor-scoped `MCP Connections` in `Data Core -> Connectors`, starting with Amplitude presets for public `https://` Streamable HTTP servers, plus OAuth authorization, read-only tool discovery, `Open In Ask AI`, explicit `Import Snapshot`, and draft cohort creation from imported identifier rows
...
- a prompt-driven `Insight Copilot` operator agent that can use backend-managed Gemini, OpenAI, or Anthropic model profiles, with Gemini, LM Studio, Ollama, and custom OpenAI-compatible presets managed from `Data Core -> Connectors -> AI Agents & Models`; OpenAI-compatible endpoints must be reachable from the backend runtime, so localhost-style presets are for self-hosted or local deployments; the agent can also use a selected `MCP Connection` for read-only remote tool runs, then import the result as an MCP snapshot or turn imported identifier rows into a draft cohort, alongside the existing SQL drafting, guided audience-builder drafting, prediction reuse, and draft campaign/workflow setup flows
