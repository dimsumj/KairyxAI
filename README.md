...
- `Action Orchestrator` for provider-aware lifecycle email campaigns, push and email workflows, delivery, callbacks, and guardrails
...
- provider-aware lifecycle delivery in `Action Orchestrator`, including tenant-scoped provider connections managed in `Data Core -> Connectors` through a `Connect Campaign Provider` action, row-level provider deletion guarded against active campaign references, SendGrid dynamic template browsing, Braze API campaign selection, Wynn PushNotifier workflow delivery, prediction-or-cohort audience targeting, human-readable audience labels, sampled JSON-key recipient field selectors, optional prediction risk filters, deeplink merge-field injection, push payload fields for `campaign_name`, `title`, `body`, `deep_link`, `deep_link_token`, JSON `data`, optional schedule override, simulator fallback when no Wynn connection is selected, and separate upcoming versus past email campaign views
- a prompt-driven `Insight Copilot` operator agent that can use backend-managed Gemini, OpenAI, or Anthropic model profiles, with Gemini, LM Studio, Ollama, and custom OpenAI-compatible presets managed from `Data Core -> Connectors -> AI Agents & Models`, draft SQL from prediction context, reuse or start prediction jobs, and create draft cohorts, email campaigns, and workflows from one chat flow while keeping publish, send, and other live actions confirmation-gated or manual
